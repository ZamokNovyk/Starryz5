...
import { 
  ProfessorNotificationPreferences, 
  getProfessorNotificationPreferences, 
  saveProfessorNotificationPreferences, 
  removeProfessorNotificationSubscription 
} from '@/src/lib/professors';

interface ProfessorNotificationModalProps {
  isOpen: boolean;
  onClose: () => void;
  professorId: string;
...
export default function ProfessorNotificationModal({
  isOpen,
  onClose,
  professorId,
  professorName,
...
}: ProfessorNotificationModalProps) {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [successSaved, setSuccessSaved] = useState(false);

  const [preferences, setPreferences] = useState<ProfessorNotificationPreferences>({
    notify_crush: true,
    notify_review: true,
    notify_known: true,
    notify_fan: true
  ...
  useEffect(() => {
    if (!isOpen || !professorId || !userUid) return;

    let isMounted = true;
    setLoading(true);
    setSuccessSaved(false);

    getProfessorNotificationPreferences(professorId, userUid)
      .then((prefs) => {
        if (isMounted) {
          if (prefs) {
  ...
  }, [isOpen, professorId, userUid]);

  if (!isOpen) return null;

  const hasAnyActive = Object.values(preferences).some(Boolean);

  const handleToggle = (key: keyof ProfessorNotificationPreferences) => {
    setPreferences((prev) => ({
      ...prev,
      [key]: !prev[key]
    }));
  ...
  const handleSave = async () => {
    ...
    try {
      if (!hasAnyActive) {
        await removeProfessorNotificationSubscription(professorId, userUid);
        if (onSubscriptionChange) onSubscriptionChange(false);
      } else {
        await saveProfessorNotificationPreferences(professorId, userUid, preferences);
      ...
      console.error('Error guardando preferencias de profesor:', e);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDisableAll = async () => {
    ...
    try {
      ...
      console.error('Error desactivando suscripción de profesor:', e);
    } finally {
      setSaving(false);
    }
  };
      ...
      >
        ...
        <div className="flex items-center gap-4 pr-8">
          <div className="relative">
            <div className="w-14 h-14 rounded-2xl overflow-hidden bg-zinc-900 border border-amber-500/30 flex items-center justify-center flex-shrink-0 shadow-lg">
              {professorAvatar ? (
                <img src={professorAvatar} alt={professorName} className="w-full h-full object-cover" />
              ) : (
          ...
          <div className="space-y-0.5">
            ...
            <p className="text-xs text-zinc-400">
              Elige qué avisos deseas recibir sobre este profesor.
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-zinc-400 leading-relaxed">
                    Notificarme cuando este profesor reciba un nuevo flechazo o cuando el contador de crushes cambie.
                  </p>
                </div>
              </div>
            ...
            {/* Opción 2: Reseñas y Calificaciones */}
            <div 
              onClick={() => handleToggle('notify_review')}
              className={`p-4 rounded-2xl border transition-all cursor-pointer select-none flex items-start justify-between gap-3 ${
                preferences.notify_review 
            ...
            >
              <div className="flex items-start gap-3">
                <div className={`p-2.5 rounded-xl flex items-center justify-center flex-shrink-0 ${
                  preferences.notify_review ? 'bg-amber-500/20 text-[#eab308]' : 'bg-zinc-900 text-zinc-500'
                }`}>
                  <Star className={`w-5 h-5 ${preferences.notify_review ? 'fill-amber-500 text-amber-500' : ''}`} />
                </div>
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <h4 className="text-xs font-bold text-white uppercase tracking-wider">
                  ...
                  <p className="text-[11px] text-zinc-400 leading-relaxed">
                    Notificarme cuando califiquen a este profesor o se publiquen nuevas valoraciones y comentarios sobre sus clases.
                  </p>
                </div>
              </div>

              <div className={`w-11 h-6 rounded-full transition-colors relative flex items-center flex-shrink-0 p-0.5 cursor-pointer ${
                preferences.notify_review ? 'bg-[#eab308]' : 'bg-zinc-800'
              }`}>
                <div className={`w-5 h-5 rounded-full bg-white transition-transform ${
                  preferences.notify_review ? 'translate-x-5' : 'translate-x-0'
                }`} />
              </div>
            </div>
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-zinc-400 leading-relaxed">
                    Avisarme cuando alumnos del campus confirmen que conocen o han llevado clases con este profesor.
                  </p>
                </div>
              </div>
            ...
            >
              <div className="flex items-start gap-3">
                <div className={`p-2.5 rounded-xl flex items-center justify-center flex-shrink-0 ${
                  preferences.notify_fan ? 'bg-purple-500/20 text-purple-400' : 'bg-zinc-900 text-zinc-500'
                }`}>
              ...
              <div className={`w-11 h-6 rounded-full transition-colors relative flex items-center flex-shrink-0 p-0.5 cursor-pointer ${
                preferences.notify_fan ? 'bg-purple-500' : 'bg-zinc-800'
              }`}>
                <div className={`w-5 h-5 rounded-full bg-white transition-transform ${
        ...
        <div className="space-y-2 pt-2 border-t border-zinc-800/80">
          ...
          {hasAnyActive && (
            ...
            >
              Desactivar todas las alertas de este profesor
            </button>
          )}
        </div>
      </div>
